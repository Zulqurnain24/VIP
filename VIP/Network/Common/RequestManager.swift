        ...
        guard var url = request.url else {
            ...
            request.completion?(
                Result.failure(ResultError
                                ...
                                "Error creating URL for endpoint: \(String(describing: request))")))
            return
        }
        ...
        let task = session.dataTask(with: httpRequest) { (serverData, responseData, error) in
            DispatchQueue.main.async {
                ...
                if !httpCode.isSuccess {
                    ...
                    dataDict.updateValue(String(data:
                                                    (serverData ?? Data()),
                                                encoding: .utf8) ?? "", forKey: "Response")
                ...
                if let processHeader = request.processHeader,
                   processHeader == true,
                   let responseData = responseData as? HTTPURLResponse {
                    informationData = try? JSONSerialization.data(withJSONObject: responseData.allHeaderFields,
